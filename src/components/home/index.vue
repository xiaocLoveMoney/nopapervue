    ...
    <div>
                            ...
                            <el-row>
                                <el-col :span="16" style="padding: 1%">
                                    <el-descriptions :column="2" size="large" direction="vertical">
                                        ...
                                        <el-descriptions-item label="提交考勤状态">
                                            <el-button type="success"
                                                       :disabled="info.role=='normal' || students.length == todayAttendance.length"
                                                       @click="checkVisible = !checkVisible; console.log(students.length, todayAttendance.length)">提交考勤状态
                                            </el-button>
                                        </el-descriptions-item>
                                    </el-descriptions>
                                </el-col>
        ...
        <el-dialog v-model="checkVisible" title="老师提交全班考勤状态" width="800" draggable>
            ...
            <el-collapse>
                ...
                <el-empty description="暂无请假数据" v-show="todayAttendance.length == 0 && getleaveserror.length == 0 || todayAttendance.length == 0"/>
                <el-tag type="danger" v-for="(item,index) in getleaveserror" :key="index" style="margin: 10px" v-show="item.status=='正常' && todayAttendance.length != 0"> {{ item.tag }}</el-tag>
            </el-collapse>
            <template #footer>
                <div class="dialog-footer">
        ...
        <!--    审批请假    -->
        <el-dialog v-model="checkleavesVisible" title="老师提交全班考勤状态" width="800" draggable>
            <!--            <span>提交学生请假申请</span>-->

            <el-collapse>
                <el-collapse-item :title="singleper.row.name">
                    <div v-show="singleper.row.resion != null">
                        <span style="font-weight: bold">请假原因:</span>
                        <div>{{ singleper.row.resion }}</div>
                        <el-radio-group v-model="teachecked">
                            <el-radio :value="false">拒批</el-radio>
                            <el-radio :value="true">批准</el-radio>
                        </el-radio-group>
                    </div>
                    <el-empty description="暂无数据" v-show="singleper.row.resion == null"/>
                </el-collapse-item>
            ...
            <template #footer>
                <div class="dialog-footer">
                    ...
                    <el-popover
                        ...
                        :width="250"
                        trigger="hover"
                        content="选择（批准/拒批）后提交即可">
                        <template #reference>
                            <el-button type="primary" @click="leaveschecks">提交</el-button>
                        </template>
                    </el-popover>
                </div>
            </template>
...
import {
    ...
    info,
    student_createLeaves, teacheract, upload_attendance
} from "@/API/auth";
...
export default {
    ...
    methods: {
        ...
        tableclick(row, column, cell, event) {
            ...
            if (row.tag == "正常到校" || row.tag == '已审批') {
                ElMessage({
                    "message": "该学生并没有需要审批的内容哦",
                    "type": "warning",
                })
                return;
            }
            this.checkleavesVisible = true;
            this.singleper.row = row
            this.singleper.column = column
            this.singleper.cell = cell
            this.singleper.event = event
        },
        leaveschecks() {
            console.log(123123)
            teacheract({
                "attendance_id": this.singleper.row.attendance_id,
                "status": '2',
            }).then((res) => {
                console.log(res);
                ElMessage({
